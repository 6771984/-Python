...
class Cell:
    ...
    def __add__(self, other):
        return f'Сумма клеток = {(self.qty + other.qty)}'

    def __sub__(self, other):
        if (self.qty - other.qty) > 0:
            return f'Разность клеток = {(self.qty - other.qty)}'
        return f'Разность отрицательна, поэтому операция не выполняется'

    def __mul__(self, other):
        return f'Умножение клеток = {(self.qty * other.qty)}'

    def __truediv__(self, other):
        return f'Деление клеток = {(self.qty // other.qty)}'
